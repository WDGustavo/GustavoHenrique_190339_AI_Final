...
public class AI : MonoBehaviour
{
    ...
    public float RunDistance = 80.0f;
    ...
    [Task]
    //metodo para fazer o bot fugir
    public void Fugir()
    {
        //faz o calculo entre a distancia dele e do player
        float distance = Vector3.Distance(transform.position, player.transform.position);
        //se a distancia for menor que o run distance
        if (distance < RunDistance)
        {
            //faz a distancia dele menos a distancia do player
            Vector3 posicaodoPlayer = player.transform.position - transform.position;
            //seta a nova posição dele
            Vector3 novaPosicao = transform.position - posicaodoPlayer;
            //faz ele ir para a nova posição
            agent.SetDestination(novaPosicao);
            //almenta a velociade para 30
            agent.speed = 30;
        }
        //se a distancia for maior que o run distance
        if (distance > RunDistance)
        {
            //fala que a ação foi concluida com sucesso
            Task.current.Succeed();
